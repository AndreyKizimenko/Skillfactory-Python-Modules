...
def test_get_all_pets_with_valid_key(_filter=''):
    _, auth_key = pf.get_api_key(user_email, user_password)
    status, result = pf.get_list_of_pets(auth_key, _filter)

    assert status == 200
    assert len(result['pets']) > 0

def test_add_new_pet_with_valid_data(name='Барбоскин', animal_type='двортерьер',
                                     age='4', pet_photo='images/Ostrich.jpg'):
...
def test_get_all_pets_with_an_invalid_key(_filter=''):
    _, auth_key = pf.get_api_key(user_email, user_password)
    status, result = pf.get_list_of_pets(auth_key, _filter)
